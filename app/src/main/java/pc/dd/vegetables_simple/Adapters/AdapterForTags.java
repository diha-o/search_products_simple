...
public class AdapterForTags  extends BaseAdapter {
    ...
    public AdapterForTags(View c, ArrayList list2) {
        mview = c;
        list= list2;
    }
    ...
    private View createOneCategory(final View v, String category, final int id){
        //add checked if clicked
        chekClickedCategory.put(id,false);

        //add layout
        final LinearLayout temp = new LinearLayout(v.getContext());
       ...
       // temp.setLayoutParams(layoutParams);
        ...
        //layout settings
        ColorDrawable cd = new ColorDrawable(ContextCompat.getColor(v.getContext(), R.color.colorAccent)); // color layout //background
        temp.setBackground(cd);
        temp.setPadding(10,8,10,8);
        temp.setGravity(Gravity.CENTER);

        //add text
        final TextView mCategory = new TextView(v.getContext());
        mCategory .setText(category);
        mCategory .setId(id*100); // R.id settings
        mCategory .setLayoutParams(new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.WRAP_CONTENT,
                LinearLayout.LayoutParams.WRAP_CONTENT));
        mCategory.setTextColor(ContextCompat.getColor(v.getContext(), R.color.white));
    ...
    public void categoryClick(LinearLayout colorLayout, View root, Boolean cheked,int id,String category_name){
        ColorDrawable cd;
        chekClickedCategory.remove(id);//remove position
        if(!cheked) {
            chekClickedCategory.put(id,true); //add position again
            cd = new ColorDrawable(ContextCompat.getColor(root.getContext(), R.color.colorClickedCategory));
            category_for_search.add(category_name); //add category for search
        }
        else{
            chekClickedCategory.put(id,false); //add
            cd = new ColorDrawable(ContextCompat.getColor(root.getContext(), R.color.colorAccent));
